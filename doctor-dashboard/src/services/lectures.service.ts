...
// Schedule type for my-schedule endpoint
...
export const lecturesService = {
  ...
  // Get my schedule (for doctor)
  getMySchedule: async (): Promise<WeekSchedule> => {
    const response = await api.get<ApiResponse<WeekSchedule>>("/lectures/my-schedule");
    return response.data.data;
  },

  getToday: async (): Promise<Lecture[]> => {
    const response = await api.get<ApiResponse<Lecture[]>>("/lectures/today");
    return response.data.data;
  },
